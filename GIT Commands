Repository - A repository is a data structure that stores project or sets of project files. 
...
Clone - Clone is a git command that clones an existing repository and puts this new repository in a new local directory. 
With a clone, a developer can edit a file in their preferred file editor or development environment. 

Fork - Forking allows you to clone a repository on the gitHub server side. 
Then, you can clone it to your local system and push your new repository upstream. 

Branch - A branch is essentially a lightweight pointer to a commit. 
...
Commit - A commit is essentially a revision to a file or set of files. 
...
Merge - What a merge does is take changes made from one branch and apply them to another branch. 
This is the git command software engineers can use to merge changes made in their code with their main code. 

Checkout - The checkout command allows you to update a repositorie’s state to a specific point in the project’s development 

Push - the push command allows you send changes in code to remote repositories. 
For example, if an organization has a repository on Github a developer may push their changes to it.   

Pull - the pull command allows you to fetch new code from other remote files and merge them with your current work. 

Remote Add / Remove / Show - These are the commands you use to work with remotes. 
...
Status - The status command analyzes the current repository and determines whether or not the current repository needs any items committed. 
...
Master Branch - Master is a default name for the original local branch of a project. 
